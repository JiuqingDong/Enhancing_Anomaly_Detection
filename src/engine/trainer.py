...
class Trainer():
    ...
    def train_classifier(self, train_loader, val_loader, test_loader):
        ...
        for epoch in range(total_epoch):
            ...
            if curr_acc > best_metric:
                ...
                out_path = os.path.join(self.cfg.OUTPUT_DIR, f"{t_name}_best_model.pth")
            ...
            #     model_state = self.model.state_dict()
            #     out_path = os.path.join(self.cfg.OUTPUT_DIR, f"{t_name}_last_model.pth")
            #     torch.save(model_state, out_path)
            #     logger.info(f"Saved model for {t_name} at {out_path}")
            #     self.ood_evaluator(test_loader, epoch=epoch)
            # else:
    ...
    def eval_classifier(self, data_loader, prefix, save=True):
        ...
        for idx, input_data in enumerate(data_loader):
            ...
            loss, outputs, feature = self.forward_one_batch(X, targets, False)
    ...
    def ood_evaluator(self, data_loader, epoch = None, T = 1):
        ...
        for idx, input_data in enumerate(data_loader):
            ...
            # Energy
            InD_score_energy.append(-to_np((T*torch.logsumexp(outputs / T, dim=1))))
            # entropy
            ...
            InD_score_max_logits.append(-np.max(max_logit, axis=1))

            # # 计算logits的和，这会给出一个形状为[N,]的数组，每个元素是对应样本的logits之和
            # max_logit = to_np(outputs / T)
            # sum_of_logits = np.sum(max_logit, axis=1, keepdims=True)
            # max_logit = max_logit / sum_of_logits
            # InD_score_max_logits.append(-np.max(max_logit, axis=1))
        ...
        for name, ood_loader in self.ood_dataset_dic.items():
            ...
            for idx, input_data in enumerate(ood_loader):
                ...
                smax = to_np(F.softmax(outputs / T, dim=1))
                # energy
                OOD_score_energy.append(-to_np((T * torch.logsumexp(outputs / T, dim=1))))
                # entropy
                ...
                # max_logits
                max_logit = to_np(outputs/T)
                OOD_score_max_logits.append(-np.max(max_logit, axis=1))
