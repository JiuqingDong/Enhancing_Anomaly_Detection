...
class JSONDataset(torch.utils.data.Dataset):
    ...
    def get_anno(self):
        ...
        if self.data_percentage < 1.0:
            if self.name == 'plant_village' or self.name == 'pvt':
...
class PVT(JSONDataset):
    """Nabirds dataset."""

    def __init__(self, cfg, split):
        super(PVT, self).__init__(cfg, split)

    def get_imagedir(self):
        return self.data_dir

class HERBARIUM_19(JSONDataset):
    """Nabirds dataset."""

    def __init__(self, cfg, split):
        super(HERBARIUM_19, self).__init__(cfg, split)
